                               ...
                               boost::shared_ptr<aerial_robot_estimation::StateEstimator> estimator)
{
  ...
  flight_status_ack_sub_ = nh_.subscribe("flight_config_ack", 1, &BaseNavigator::flightStatusAckCallback, this,
...
void BaseNavigator::batteryCheckCallback(const std_msgs::Float32ConstPtr& msg)
{
  if (std::isnan(msg->data))
  {
    throw std::runtime_error(
        "the voltage from spinal is Nan, please re-calibrate the voltage scale using /set_adc_scale.");
  }
  ...
  float percentage = 0;
  if (average_voltage > VOLTAGE_90P)
    percentage = (average_voltage - VOLTAGE_90P) / (VOLTAGE_100P - VOLTAGE_90P) * 10 + 90;
  else if (average_voltage > VOLTAGE_80P)
    percentage = (average_voltage - VOLTAGE_80P) / (VOLTAGE_90P - VOLTAGE_80P) * 10 + 80;
  else if (average_voltage > VOLTAGE_70P)
    percentage = (average_voltage - VOLTAGE_70P) / (VOLTAGE_80P - VOLTAGE_70P) * 10 + 70;
  else if (average_voltage > VOLTAGE_60P)
    percentage = (average_voltage - VOLTAGE_60P) / (VOLTAGE_70P - VOLTAGE_60P) * 10 + 60;
  else if (average_voltage > VOLTAGE_50P)
    percentage = (average_voltage - VOLTAGE_50P) / (VOLTAGE_60P - VOLTAGE_50P) * 10 + 50;
  else if (average_voltage > VOLTAGE_40P)
    percentage = (average_voltage - VOLTAGE_40P) / (VOLTAGE_50P - VOLTAGE_40P) * 10 + 40;
  else if (average_voltage > VOLTAGE_30P)
    percentage = (average_voltage - VOLTAGE_30P) / (VOLTAGE_40P - VOLTAGE_30P) * 10 + 30;
  else if (average_voltage > VOLTAGE_20P)
    percentage = (average_voltage - VOLTAGE_20P) / (VOLTAGE_30P - VOLTAGE_20P) * 10 + 20;
  else if (average_voltage > VOLTAGE_10P)
    percentage = (average_voltage - VOLTAGE_10P) / (VOLTAGE_20P - VOLTAGE_10P) * 10 + 10;
  else
    percentage = (average_voltage - VOLTAGE_0P) / (VOLTAGE_10P - VOLTAGE_0P) * 10;
  ...
  if (power_info_pub_.getNumSubscribers() == 0)
    return;
  ...
  switch (msg->pos_xy_nav_mode)
  {
    case aerial_robot_msgs::FlightNav::POS_MODE: {
      ...
      if (msg->target == aerial_robot_msgs::FlightNav::BASELINK)
      {
        ...
        target_cog_pos -= tf::Matrix3x3(tf::createQuaternionFromYaw(getTargetRPY().z())) * cog2baselink_tf.getOrigin();
      }
...
//    }
    ...
    case ACC_CONTROL_MODE: {
      if (teleop_flag_)
      {
        ...
        /* acc command */
        target_acc_.setValue(
            joy_cmd.axes[PS3_AXIS_STICK_LEFT_UPWARDS] * max_target_tilt_angle_ * aerial_robot_estimation::G,
        ...
        if (control_frame_ == LOCAL_FRAME)
        {
          ...
          tf::transformKDLToTF(segments_tf.at(robot_model_->getBaselinkName()).Inverse() *
                                   segments_tf.at(teleop_local_frame_),
    ...
    case VEL_CONTROL_MODE: {
      if (teleop_flag_)
      {
        ...
        tf::Vector3 target_vel(joy_cmd.axes[PS3_AXIS_STICK_LEFT_UPWARDS] *
                                   fabs(joy_cmd.axes[PS3_AXIS_STICK_LEFT_UPWARDS]) * max_target_vel_,
                               joy_cmd.axes[PS3_AXIS_STICK_LEFT_LEFTWARDS] *
                                   fabs(joy_cmd.axes[PS3_AXIS_STICK_LEFT_LEFTWARDS]) * max_target_vel_,
                               0);
...
void BaseNavigator::update()
{
  ...
  else
  {
    ...
    if (!estimator_->getStateStatus(State::X_BASE, estimate_mode_) ||
        !estimator_->getStateStatus(State::Y_BASE, estimate_mode_))
    {
    ...
    case HOVER_STATE: {
      ...
      if (gps_waypoint_)
      {
        if (ros::Time::now().toSec() - gps_waypoint_time_ > gps_waypoint_check_du_)
        {
          ...
          tf::Matrix3x3 convert_frame;
          convert_frame.setRPY(M_PI, 0, 0);  // NED -> XYZ
      ...
      if (vel_based_waypoint_)
      {
        ...
        else
        {
          if (gps_waypoint_)
          {
            ...
            ROS_WARN("back to pos nav control for GPS way point, gps waypoint delta: %f, %f", gps_waypoint_delta.x(),
                     gps_waypoint_delta.y());
            gps_waypoint_ = false;
          }
          else
          {
...
void BaseNavigator::rosParamInit()
{
  ...
  getParam<double>(bat_nh, "bat_resistance_voltage_rate", bat_resistance_voltage_rate_,
                   0.0);                                                 // Battery internal resistance_voltage_rate
