...
class Visualizer:
    ...
    def visualize(
        ...
        if self.tilt:
            ...
            plt.plot(time_data_x, r_sim_all[:self.data_idx, 9], label="gimbal1f")
            plt.plot(time_data_x, r_sim_all[:self.data_idx, 10], label="gimbal2f")
            plt.plot(time_data_x, r_sim_all[:self.data_idx, 11], label="gimbal3f")   
            plt.plot(time_data_x, r_sim_all[:self.data_idx, 12], label="gimbal4f")
            plt.legend(framealpha=legend_alpha)
            # plt.xlabel("Time (s)")
            plt.xlim([0, t_total_sim])
